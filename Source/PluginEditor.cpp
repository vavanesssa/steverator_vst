...
//==============================================================================
juce::Rectangle<int> Vst_saturatorAudioProcessorEditor::scaleDesignBounds(int x, int y, int width, int height) const
{
...
void Vst_saturatorAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.fillAll(juce::Colour::fromFloatRGBA(0.15f, 0.15f, 0.15f, 1.0f)); // Dark grey for letterbox

    // Apply global transform for scaled content area
    juce::Graphics::ScopedSaveState saveState(g);
    g.addTransform(juce::AffineTransform::scale(scaleFactor).translated(static_cast<float>(offsetX), static_cast<float>(offsetY)));
    ...
    juce::Rectangle<int> versionBox(DESIGN_WIDTH - 350, DESIGN_HEIGHT - 32, 340, 26);
...
void Vst_saturatorAudioProcessorEditor::resized()
{
    ...
    float scaleX = static_cast<float>(windowWidth) / static_cast<float>(DESIGN_WIDTH);
    float scaleY = static_cast<float>(windowHeight) / static_cast<float>(DESIGN_HEIGHT);
    ...
    inputGainSlider.setBounds(scaleDesignBounds(col1X + 5, row2Y, knobWidth, knobHeight));
    outputGainSlider.setBounds(scaleDesignBounds(col1X + 5, row3Y, knobWidth, knobHeight));

    // COLUMN 2: LOW BAND
    lowEnableButton.setBounds(scaleDesignBounds(col2X + (columnWidth - 110) / 2, buttonRowY, 110, buttonHeight));
    lowFreqSlider.setBounds(scaleDesignBounds(col2X + 5, row1Y, knobWidth, knobHeight));
    lowWarmthSlider.setBounds(scaleDesignBounds(col2X + 5, row2Y, knobWidth, knobHeight));
    lowLevelSlider.setBounds(scaleDesignBounds(col2X + 5, row3Y, knobWidth, knobHeight));

    // COLUMN 3: HIGH BAND
    highEnableButton.setBounds(scaleDesignBounds(col3X + (columnWidth - 110) / 2, buttonRowY, 110, buttonHeight));
    highFreqSlider.setBounds(scaleDesignBounds(col3X + 5, row1Y, knobWidth, knobHeight));
    ...
    bypassButton.setBounds(scaleDesignBounds(footerStartX, footerY, footerButtonWidth, footerButtonHeight));
    limiterButton.setBounds(scaleDesignBounds(footerStartX + footerButtonWidth + columnSpacing / 2, footerY, footerButtonWidth, footerButtonHeight));
    prePostButton.setBounds(scaleDesignBounds(footerStartX + (footerButtonWidth + columnSpacing / 2) * 2, footerY, footerButtonWidth, footerButtonHeight));
